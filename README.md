...
- `--tag vX.Y.Z`: the release to pin to, for a local run; by default the newest `v*` tag reachable from `HEAD` (`git describe --tags --abbrev=0 --match 'v*'`).
- `--shell FILE`: the page shell (below); by default a built-in shell in the ParkviewLab brand.
- `--repo DIR`: the repo to build; by default the current directory, which is where the pages workflow runs. A checkout of dev-tools in a subfolder of that repo is not read.
